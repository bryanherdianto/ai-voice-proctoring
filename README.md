...
2. **MediaPipe + WebRTC VAD + Resemblyzer**: This approach uses MediaPipe for face detection, WebRTC VAD for voice activity detection, and Resemblyzer for speaker verification. It is a simpler alternative to the first approach, focusing on real-time processing with less computational overhead. Resemblyzer is used for offline processing but is significantly faster than Pyannote.Audio and can be run in parallel for improved performance. This system doesn't do phoneme-level verification, hence might be fooled by random movements of lips in accordance with the audio.
...
The final choice of approach for this project is the second one, i.e., MediaPipe + WebRTC VAD + Resemblyzer, due to its balance between performance and computational efficiency. However, this code currently only focuses on double voice detection using Resemblyzer, while MediaPipe and WebRTC VAD are yet to be implemented for lip-sync verification.

For detailed usage instructions and API documentation, see the [Double Voice Detection Module Documentation](main/README.md).
